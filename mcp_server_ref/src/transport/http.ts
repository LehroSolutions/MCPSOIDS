...
export class HttpTransport {
  ...
  constructor(
    ...
    this.app.get('/', async (_req, res) => {
      try {
        res
          .type('html')
          .send(await fs.readFile(path.join(process.cwd(), 'public', 'index.html'), 'utf8'));
      } catch {
        res.status(404).send('Control Center asset not found.');
      }
    });
    ...
    if (authToken) {
      this.app.use((req, res, next) => {
        ...
        if (provided.length !== expected.length || !timingSafeEqual(provided, expected)) {
          res
            .status(401)
            .json(createErrorResponse(null, JsonRpcErrorCode.InvalidRequest, 'Unauthorized'));
          return;
        }
        next();
      });
    ...
    this.app.use(
      (err: any, _req: express.Request, res: express.Response, next: express.NextFunction) => {
        ...
        if (err?.type === 'entity.too.large' || err?.status === 413) {
          res
            .status(413)
            .json(createErrorResponse(null, JsonRpcErrorCode.InvalidRequest, 'Request too large'));
          return;
        }
        next(err);
      },
  ...
  private handleMcpRequest = async (req: express.Request, res: express.Response) => {
    ...
    if (this.healthMonitor.getActiveRequests() > this.maxActiveRequests) {
      ...
      res
        .status(429)
        .json(createErrorResponse(null, JsonRpcErrorCode.Timeout, 'Too Many Requests'));
      return;
    }
    ...
    try {
      ...
      if (!parseResult.success) {
        res.json(
          createErrorResponse(null, JsonRpcErrorCode.InvalidRequest, 'Invalid JSON-RPC request'),
        );
        return;
      }
  ...
  private handleAuditEntries = async (req: express.Request, res: express.Response) => {
    try {
      ...
      res
        .status(500)
        .json(
          createErrorResponse(null, JsonRpcErrorCode.InternalError, 'Failed to read audit entries'),
        );
    }
  };
  ...
  private async dispatchToolCall(params: any, requestId: string | number | null) {
    ...
    if (typeof toolName !== 'string' || toolName.length === 0) {
      throw {
        ...
        message: 'Invalid params: tools/call requires a tool name',
      };
    }
  ...
  private normalizeToolError(err: any) {
    ...
    if (
      err?.name === 'ZodError' ||
      (typeof err?.message === 'string' && err.message.startsWith('Security Violation'))
    ) {
      return { code: JsonRpcErrorCode.InvalidParams, message: err.message };
    }
    if (
      typeof err?.code === 'string' &&
      ['ENOENT', 'ENOTDIR', 'EISDIR', 'EACCES', 'EPERM'].includes(err.code)
    ) {
      return { code: JsonRpcErrorCode.InvalidParams, message: err.message };
    }
    return err;
  ...
  private async recordToolAudit(input: {
    ...
    try {
      ...
      this.logger.error(
        { err, auditPath: this.auditLog.getPath() },
  ...
  public start(port: number, host?: string) {
    const onListen = () => {
      this.logger.info(
        `MCP Server compliant with 'Active Robustness' listening on ${host ? host + ':' : 'port '}${port}`,
      );
    };
