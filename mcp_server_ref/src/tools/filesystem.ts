...
export const LS_TOOL_DEFINITION = {
  ...
  inputSchema: {
    ...
    properties: {
      path: {
        type: 'string',
        description: 'Directory path, must be within Roots. Defaults to current root.',
      },
    },
  },
};
...
export const READ_FILE_TOOL_DEFINITION = {
  name: 'fs/read_file',
  description:
    'Read a text file within the configured Roots with a strict size cap (prevents unbounded reads).',
  inputSchema: {
    type: 'object',
    properties: {
      ...
      max_bytes: {
        type: 'number',
        description: `Maximum bytes to read (<= ${MAX_READ_BYTES}).`,
