...
async function httpRequest(method: 'GET' | 'POST', requestPath: string, body?: string, includeAuth = true) {
...
async function runInspection() {
    ...
    if (MCP_AUTH_TOKEN) {
        ...
        if (unauth.statusCode !== 401) {
            throw new Error(`Auth check failed - expected 401 without token, received ${unauth.statusCode}`);
        }
    ...
    const hasEchoAudit = auditEntries.some((entry: any) =>
        entry.toolName === '__echo' &&
